# Specs por feature

Un documento por feature, autocontenido. La idea: cuando se implementa
una feature (normalmente lanzando a Codex una tarea acotada), el agente
lee **solo** el doc de esa feature en vez de los ~250 líneas de
`plan.md` donde el 80% no aplica a lo que está haciendo.

## Cómo se usa

Al empezar una feature, el agente lee **exactamente estos tres**:

1. `specs/features/<feature>.md` — qué construir y cómo (este directorio)
2. `AGENTS.md` — reglas del repo (seguridad, ramas, skills)
3. `specs/design-system.md` — solo si toca `src/components/`

`spec.md` y `plan.md` **no** hacen falta para implementar: el doc de
feature ya incluye lo relevante. Se consultan solo si hay que cambiar
una decisión global.

## Qué va en cada capa

| Documento | Contiene | Cambia... |
|---|---|---|
| `spec.md` | El **qué** y el **por qué** a nivel producto: alcance, modelo de datos, criterios de aceptación | Rara vez — solo si cambia el producto |
| `plan.md` | Decisiones **transversales** (stack, estructura de carpetas, convenciones de test) + índice de features | Rara vez |
| `features/<x>.md` | El **cómo** de una feature concreta: rutas, componentes, flujos, SQL, tests, checkpoints de seguridad | Una vez por feature, y se marca como cerrada |
| `design-system.md` | Paleta semántica, inventario de componentes UI | Al añadir un patrón visual nuevo |

## Convención

- Nombre: `<feature-corta>.md` en kebab-case (`auth.md`,
  `data-layer.md`, `dashboard.md`).
- Cada doc abre con **Estado** (`⏳ pendiente` / `🚧 en curso` /
  `✅ implementada en PR #N`) para saber de un vistazo qué queda.
- Cada doc incluye su propia sección de **checkpoints de seguridad**:
  qué revisar específicamente en esa feature, no el checklist genérico.
- Si una feature necesita configuración manual en Supabase/Vercel (algo
  que un agente sin red **no puede hacer**), va en una sección
  **Configuración manual requerida** bien visible al principio.

## Índice

| Feature | Doc | Estado |
|---|---|---|
| Autenticación completa (registro con verificación por código, login, recuperación de contraseña, tabla `profiles`) | [auth.md](auth.md) | ✅ código en PR #14; `0002` verificada completa en remoto |
| Capa de datos (`queries/`, hooks de React Query) | [data-layer.md](data-layer.md) | ✅ implementada en PR #15 |
| Dashboard (stats, lista de pendientes) | [public-docs.md](public-docs.md) | ✅ absorbida por el pivote, como panel admin — PR #16 |
| Categorías (índice + CRUD) | [public-docs.md](public-docs.md) | ✅ implementada en PR #16 |
| Tecnologías (ficha, formulario, badges) | [public-docs.md](public-docs.md) | ✅ implementada en PR #16 |
| Documentación pública, comentarios y favoritos (pivote de alcance) | [public-docs.md](public-docs.md) | ✅ implementada en PR #16 — migración `0003` aplicada y verificada en remoto, probado en vivo con 3 identidades reales |
| 2FA con TOTP (segundo factor opcional en login) | _pendiente de escribir_ | ⏳ aplazada a propósito, ver [auth.md](auth.md#fuera-de-alcance-de-esta-feature) |
| Iconos de categoría y tecnología (`IconPicker`, lucide-react + react-icons/si) | [icons.md](icons.md) | ✅ implementada en PR #20 |
| Lecciones: tercer nivel del contenido (categoría → tecnología → lección), `LeccionPage`, comentarios anclados a lección | [lecciones.md](lecciones.md) | ✅ implementada — migraciones `0004`/`0007` aplicadas |
| Progreso personal (estado + lección actual por tecnología en `user_technology_progress`; estado por lección individual en `user_leccion_progress`, con el estado de la tecnología ahora derivado de ese progreso en vez de manual) | [progress.md](progress.md) | ✅ implementada en PR #26 — migración `0006` aplicada y verificada con curl real (12/12 casos, 3 identidades); extendida en PR #40/#41 — migraciones `0008`/`0009` |
| Bloques de laboratorio en lecciones (`predice-el-resultado`, `codigo-anotado`, `comparador-antes-despues`, embebidos vía ` ```laboratorio ` en el Markdown) | [laboratorios.md](laboratorios.md) | ✅ implementada — contenido de las 2 lecciones piloto de HTML actualizado y verificado en producción |
| Tarjetas y cabeceras con banda de color de marca (`TechnologyCard`, hero de `TechnologyPage`, bandas de módulo) | [design-system.md](../design-system.md) | ✅ implementada en PR #39 |
| Editor en vivo (bloque `editor-en-vivo` con CodeMirror 6 + preview en iframe) y Proyectos (`lecciones.es_proyecto`, ruta `/proyectos`) | [editor-en-vivo.md](editor-en-vivo.md) | ✅ implementada — 160 lecciones + 12 proyectos nuevos reescritos; pendiente aplicar migración `0010` a producción (usuario) y verificación visual (sin Playwright en este entorno) |
| TypeScript: temario propio (~12 módulos / 40-48 lecciones) + compilación real (`typescript`+`@typescript/vfs`) en el editor en vivo | [typescript-compilacion-en-vivo.md](typescript-compilacion-en-vivo.md) | 🚧 en curso — mecanismo implementado y verificado (esquema, compilador real, pestaña `ts`); pendiente el temario y las lecciones |
| SQL: nueva categoría "Bases de datos" + dos bloques nuevos (`sql-anotado`, `sql-en-vivo`) que ejecutan consultas reales contra SQLite/WASM (`sql.js`), con verificación de resultado, + 45 lecciones en 11 módulos | [sql-en-vivo.md](sql-en-vivo.md) | ✅ implementada — mecanismo y contenido completos, publicados |
| PostgreSQL: nueva tecnología (misma categoría "Bases de datos") + extensión de `sql-anotado`/`sql-en-vivo` con `motor: 'postgres'` (PGlite/WASM real, incluida RLS real con identidad simulada) + 61 lecciones en 19 módulos | [postgresql-en-vivo.md](postgresql-en-vivo.md) | ✅ implementada — mecanismo, 61 lecciones y 4 proyectos avanzados completos, publicados (PR #43) |
| Git: nueva tecnología (categoría "Herramientas") + dos bloques nuevos (`git-anotado`, `git-en-vivo`) que ejecutan comandos reales contra libgit2/WASM (`wasm-git`), más `GrafoCommits` (grafo de commits derivado de datos reales) + 49 lecciones en 16 módulos | [git-en-vivo.md](git-en-vivo.md) | 📝 diseño aprobado y validado empíricamente, pendiente de implementar |
