...
3. **Crea dos archivos** dentro, con el icono de "Nuevo archivo" del explorador (o clic derecho → `New File`): `index.html` y `style.css` — los nombres importan, son los que enlazas en los siguientes pasos.
4. **Escribe en `index.html` la estructura completa** que ya viste en el temario de HTML (doctype, `html`, `head` con charset y viewport, `body`) — el editor en vivo de abajo te la esconde por ti, pero en un archivo real hace falta escribirla:
   ...
   <html lang="es">
     <head>
       ...
       <title>Tarjeta con micro-interacciones</title>
   ...
   ```
5. **Copia el código de partida** de la pestaña HTML del editor en vivo de abajo dentro de `<body>` (donde dice el comentario), y el de la pestaña CSS en `style.css` — es el mismo código, solo que ahora en archivos de verdad, no en un widget. El `<link>` que ya has puesto arriba es el que enlaza los dos archivos entre sí — eso el editor en vivo lo hace por ti, pero en un proyecto real hay que escribirlo a mano.
